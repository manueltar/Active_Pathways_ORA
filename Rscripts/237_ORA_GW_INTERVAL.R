      ...
      if(length(gmt_sel_GREP) >0)
      {
        ...
        for(iteration_gmt_sel_GREP in 1:length(gmt_sel_GREP))
        {
          ...
          if(dim(reference_gmt_sel)[1] >= minGS_size & dim(reference_gmt_sel)[1] <= maxGS_size)
          {
            ...
            # # gmt_sel_GREP
            # 
            # indx<-which(unique(reference_gmt_sel$id)%in%c("CUX1_TARGET_GENES","GOBP_RESPONSE_TO_ORGANIC_CYCLIC_COMPOUND","GOBP_RESPONSE_TO_IONIZING_RADIATION",
            #                                               "GOBP_CELLULAR_RESPONSE_TO_IONIZING_RADIATION","GOBP_CELLULAR_RESPONSE_TO_ORGANIC_CYCLIC_COMPOUND"))
            # 
        ...
        if(FLAG_null == 0)
        {
          ...
          if(dim(AP_result_df)[1] >0)
          {
            ...
            check<-AP_result_df_collapsed[which(AP_result_df_collapsed$id == 'CUX1_TARGET_GENES'),]
            ...
            if(DEBUG ==1)
            {
              ...
              cat(str(check))
              cat("\n")
              cat(sprintf(as.character(check$overlap_HGNC)))
              cat("\n")
            }
